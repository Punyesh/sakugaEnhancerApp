...
export interface GridClipInput {
  ...
  /**
   * When set, this clip is shorter than the target duration and should
   * freeze on its own last frame for this many extra seconds instead of
   * looping — set only when needsLoop is true and the caller chose "Stop"
   * over "Replay". Leave unset (with needsLoop true) for the normal loop
   * behavior.
   */
  stopExtendSec?: number;
  /**
   * Tokens to wrap into this clip's label (rendered as a small transparent
   * PNG on the Android side via TextPaint/Canvas, then composited with an
   * ordinary `overlay` — no drawtext/libfreetype dependency, so this works
   * on the same FFmpegKit "min" build already used for everything else).
   * Each token is kept atomic and never split mid-token: pass whole animator
   * names (joiner ", ") or whole words of custom freeform text (joiner " ").
   * Omit (or pass an empty array) for a clip with no label.
   */
  labelTokens?: string[];
  /** How labelTokens are joined when they fit on the same line. Default ", ". */
  labelJoiner?: string;
  /** Which bottom corner the label sits in. Default 'left'. */
  labelPosition?: 'left' | 'right';
  /** 'outline' (default) draws a black stroke behind white text; 'box' draws a translucent black rectangle behind it instead. */
  labelStyle?: 'outline' | 'box';
...
export interface ExportGridOptions {
  ...
  canvasHeight: number;
  /** Output duration in milliseconds — the longest clip's own duration; shorter clips loop (see {@link GridClipInput.needsLoop}) to fill it. Ignored in 'serial' format, where the total length is just the sum of every clip's own length. */
  targetDurationMs: number;
  /** Output file extension. Default `"mp4"`. */
  outputExt: string;
  /** 'grid' (default) composites every clip into one shared frame at once; 'serial' concatenates them one after another in time instead. */
  format?: 'grid' | 'serial';
}
...
export interface ExportGridResult {
  ...
  duration: number;
  /** Temporary diagnostics — number of clips a label bitmap was generated for. */
  debugLabelCount?: number;
  /** Temporary diagnostics — the exact -i/-loop/etc. arguments passed to ffmpeg. */
  debugInputArgs?: string;
  /** Temporary diagnostics — the exact -filter_complex string passed to ffmpeg. */
  debugFilterComplex?: string;
}
