...
export interface GridClipInput {
    ...
    /**
     * When set, this clip freezes on its own last frame for this many extra
     * seconds instead of looping — set only when needsLoop is true and the
     * caller chose "Stop" over "Replay".
     */
    stopExtendSec?: number;
    /**
     * Tokens to wrap into this clip's label (rendered as a small transparent
     * PNG via Android's own TextPaint/Canvas, then composited with an
     * ordinary `overlay` — no drawtext/libfreetype dependency). Each token
     * is kept atomic and never split mid-token. Omit for no label.
     */
    labelTokens?: string[];
...
export interface ExportGridOptions {
    ...
    canvasHeight: number;
    /** Output duration in milliseconds. Ignored in 'serial' format. */
    targetDurationMs: number;
    /** Output file extension. Default `"mp4"`. */
    outputExt: string;
    /** 'grid' (default) or 'serial' (concatenate clips one after another instead of compositing them together). */
    format?: 'grid' | 'serial';
}
/**
...
export interface Spec extends TurboModule {
     ...
     */
    mixAudio(videoPath: string, audioPath: string, options: MixAudioOptions): Promise<MixAudioResult>;
...
//# sourceMappingURL=NativeVideoTrim.d.ts.map